# Task: S3 Static Assets

> **Scope:** implement ONLY this node ("S3 Static Assets"). Work belonging to other nodes appears here solely as interfaces and coordination points — do not implement or re-derive it.
...
- [ ] **T2 — Expose the interface Frontend SPA (React) consumes, per Contract "Static Build Deployment (CI Upload to S3)" (custom).**
...
- [ ] **T3 — Expose the interface CloudFront CDN consumes, per Contract "CloudFront Distribution (Static Assets)" (custom).**
...
- [ ] **T5 — Resolve ownership, then implement: "Server-Side Encryption is enabled on the bucket using AWS KMS (SSE-KMS) with a customer-managed key" (REQ-006).**
  [PLACEHOLDER: owner — this node or a sharing node (S3 User Files (Versioning, SSE-KMS)); assign via the requirement mapping, then keep this task here or move it to the owning node's doc]
  ↳ serves: REQ-006 "Server-Side Encryption is enabled on the bucket using AWS KMS (SSE-KMS) with a customer-managed key"
- [ ] **T6 — Resolve ownership, then implement: "Object Versioning is enabled on buckets containing user data; deleted objects are soft-deleted via lifecycle policy for 30 days" (REQ-006).**
  [PLACEHOLDER: owner — this node or a sharing node (S3 User Files (Versioning, SSE-KMS)); assign via the requirement mapping, then keep this task here or move it to the owning node's doc]
  ↳ serves: REQ-006 "Object Versioning is enabled on buckets containing user data; deleted objects are soft-deleted via lifecycle policy for 30 days"
- [ ] **T7 — Resolve ownership, then implement: "S3 access logs are enabled and shipped to a separate logging bucket" (REQ-006).**
  [PLACEHOLDER: owner — this node or a sharing node (S3 User Files (Versioning, SSE-KMS)); assign via the requirement mapping, then keep this task here or move it to the owning node's doc]
...
- Frontend SPA (React) (initiates Static Build Deployment (CI Upload to S3) against this node (custom))
- CloudFront CDN (initiates CloudFront Distribution (Static Assets) against this node (custom))
